...
let listCache: { data: any; timestamp: number; limit: number } | null = null;
...
export const GET: APIRoute = async ({ url, locals }) => {
  ...
  if (
    listCache &&
    listCache.limit === limit &&
    Date.now() - listCache.timestamp < LIST_CACHE_TTL_MS
  ) {
    return new Response(JSON.stringify({ data: listCache.data }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  }
  ...
  const latestCfProduction = cfDeployments.find(
    (d: any) =>
      d.environment === "production" &&
      d.deployment_trigger?.metadata?.branch !== "staging"
  );
  const latestCfStaging = cfDeployments.find(
    (d: any) => d.deployment_trigger?.metadata?.branch === "staging"
  );
  ...
  if (productionRes.ok) {
    ...
    productionRuns = data.workflow_runs.map((r: any) =>
      mapRun(r, "production")
    );
  ...
  if (stagingRes.ok || productionRes.ok) {
    listCache = { data: result, timestamp: Date.now(), limit };
  }
...
export const POST: APIRoute = async ({ locals }) => {
  ...
  if (!res.ok) {
    ...
    return new Response(
      JSON.stringify({
        error: `Failed to trigger deployment: ${res.status}`,
        detail: text,
      }),
  ...
  return new Response(
    JSON.stringify({
      data: { ok: true, message: "Production deployment triggered" },
    }),
