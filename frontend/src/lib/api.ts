...
export class ApiClient {
  ...
  async login(email: string, password: string) {
    return this.request<{ id: string; email: string; name: string; roles: string[] }>(
      "/api/auth/login",
  ...
  async manageRoles(userId: string, data: { add?: string[]; remove?: string[] }) {
  ...
  async createParent(data: { email: string; name: string; phone?: string; note?: string }) {
  ...
  async updateStudent(
    id: string,
    data: { name?: string; birthday?: string; description?: string; note?: string }
  ) {
